  ...
  router.get("/user/doctor/find-radiology",function(req,res){
    ...
    router.post("/user/doctor/ecg/send-test",function(req,res){  
          ...
          if(err) throw err;        
            ...
            .exec(function(err,sessionData){
              ...
              var refNotification = {
                ...
                type: "investigation",
                date: + new Date(),
                message: "ECG investigation request.",
                sender_firstname: req.body.patient_firstname + " " + req.body.patient_lastname,
                sender_lastname: req.body.patient_lastname,
                sender_age: "",
                sender_gender: "",
        ...
        var tellPatient = function(centerInfo){
          ...
          model.user.findOne({user_id: req.body.patient_id},
            {medical_records: 1,user_id:1,patient_notification:1,presence:1,phone:1,
              accepted_doctors:1,ecg_new_indicator:1})
          .exec(function(err,record){            
            if(err) throw err;     
            var recordObj = {
              test_to_run: req.body.lab_test_list,
            ...
            var noteObj = {
              ...
              ref_id: random,
              session_id:req.body.session_id,
              message: "You have unread ECG test to run"
            }

            var elm = record.ecg_new_indicator.map(function(x){return x}).indexOf(req.user.user_id);
            if(elm == -1){
              record.ecg_new_indicator.push(req.user.user_id)
            }
...
router.get("/user/chat/general",function(req,res){
  if(req.user){
    ...
    var url = "/login?id=" + req.query.partnerId + "&type=chat";
    res.redirect(url);
  }
});
  ...
  .exec(function(err,referral){
    ...
    if(referral){  
          ...
          function updatePatient() { 
                ...
                .exec(function(err,data){
                  if(err) throw err;
                  var radioTypes = (req.body.type) ? req.body.type : 'ultrasound_test'
                  var elementPos = data.medical_records[radioTypes].map(function(x) {return x.ref_id; })
                  .indexOf(referral.ref_id);
                  var objectFound = data.medical_records[radioTypes][elementPos]; 
...
router.post('/apiAuth/v1/study',verifyApiKey, function(req,res){
  ...
  try{
    model.study.findOne({study_id: req.body.studyId, center_id: req.body.centerId})
    .exec(function(err,data){
      if(data){
        var study = data;
      } else {
...
router.get("/radiologist/dcm", function(req,res) {
  res.redirect('http://134.209.246.129:8080/applinic-mdv/home.html');
})
