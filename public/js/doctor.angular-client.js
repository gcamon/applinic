...
.when("/list2",{
 ...
 .when("/chats-notification",{
  ...
  .when("/ultra-sound-out",{
    templateUrl: 'ultra-sound-out.html',//"/assets/pages/utilities/video-notification.html",
    controller: "ultraSoundOutCtrl"
  })
  ...
  .when('/e-case-endoscopy/:id',{
    templateUrl: 'e-case-endoscopy.html',//'/assets/pages/doctor/e-case-chart.html',
  ...
  .when("/endoscopy-out",{
    templateUrl: 'endoscopy-out.html',//"/assets/pages/utilities/video-notification.html",
    controller: "endoscopyOutCtrl"
  })

  .when('/e-case-other-procedures/:id',{
    templateUrl: 'e-case-other-procedures.html',//'/assets/pages/doctor/e-case-chart.html',
    controller: 'e-caseOtherProceduresCtrl',
  })

  .when("/other-procedures-out",{
    templateUrl: 'other-procedures-out.html',//"/assets/pages/utilities/video-notification.html",
    controller: "otherProceduresOutCtrl"
  })
  ...
  function($scope,$location,$resource,$interval,localManager,templateService,requestManager,
    ...
    var getRequest = function(init) {
      ...
      var requests =  getRequestInTime.get(null,function(data){
        ...
        var investigation = filter.investigation || [];

        $rootScope.commonNotifications = labNote.concat(radioNote.concat(acceptance.concat(abnormality.concat(investigation))))
...
function($scope,$location,$rootScope,$http,ModalService,$interval,templateService,
  ...
  if(user.typeOfUser === "Patient") {
   //$interval(getAtInterval,300000)
    ...
    $scope.viewPatientChart = function(patient){
  ...
  $scope.videoChat = function(patientObj) {
    ...
    //source.get({userId: patientObj.patient_id },function(data) {
      ...
      templateService.holdForSpecificPatient = {user_id: patientObj.patient_id,
       presence: patientObj.presence, name: patientObj.patient_firstname};
      ModalService.showModal({
        templateUrl: 'sending-communication-request.html',
        controller: "videoInitController"
      }).then(function(modal) {
...
app.controller("labOutCtrl",["$scope","$http","labTests","getAllLaboratoryService","$rootScope",
  ...
  $scope.sendTest = function(center){
    for(var i = 0; i < $scope.TestList.length; i++){
      if(!$scope.TestList[i].name){
        alert('One of the test you requested has no name. Please go back and enter the test name.')
        return;
      }
    }
    center.loading = true;
...
app.controller("radioOutCtrl",["$scope","$http","scanTests","getAllRadiologyService","$rootScope",
  ...
  */
  ...
  $scope.sendTest = function(center){
    if(!$scope.TestList[count].name){
      center.loading = false;
      alert("One of the investigation you requested has no name. Please go back and enter the name.")
      return;
    }
...
app.controller("ultraSoundOutCtrl",["$scope","$http","UltraSounds","getAllRadiologyService","$rootScope",
...
app.controller("endoscopyOutCtrl",["$scope","$http","getAllRadiologyService","$rootScope",
  function($scope,$http,getAllRadiologyService,$rootScope){
...
app.controller("otherProceduresOutCtrl",["$scope","$http","getAllRadiologyService","$rootScope",
  function($scope,$http,getAllRadiologyService,$rootScope){
...
/* Endoscopy controlers logic */
app.controller("e-caseEndoscopyCtrl",["$scope","$http","$rootScope","$location",
  "templateService","ModalService","templateService","$timeout",
  function($scope,$http,$rootScope,$location,templateService,ModalService,templateService,$timeout){
...
app.controller("endoscopyModalController",["$scope","$http","UltraSounds","getAllRadiologyService","$rootScope",
  function($scope,$http,UltraSounds,getAllRadiologyService,$rootScope){
...
app.controller("e-caseOtherProceduresCtrl",["$scope","$http","$rootScope","$location",
  "templateService","ModalService","templateService","$timeout",
  function($scope,$http,$rootScope,$location,templateService,ModalService,templateService,$timeout){
...
app.controller("otherProceduresModalController",["$scope","$http","UltraSounds","getAllRadiologyService","$rootScope",
  function($scope,$http,UltraSounds,getAllRadiologyService,$rootScope){
